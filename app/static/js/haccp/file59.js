function file59Create() {
    ...
    const product_before_oil_change = $("input[name='product_before_oil_change']").val();
...
function file59Update(file59_pk) {
    const token = $("input[name='csrfmiddlewaretoken']").val();
    let data;
    const row_data = [];
    const input = [];
    ...
    console.log(data)
    $.ajax({
        url: `/haccp/api/file59/${file59_pk}/`,
        type: "PATCH",
        contentType: 'application/json',
        data: JSON.stringify(data,),
        beforeSend: function (xhr, settings) {
